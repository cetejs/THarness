...
第一阶段最小可玩闭环原型已迁移到实验一目录，运行时为 Godot .NET/C#。
...
- 已将占位视觉、截图不能替代交互、导入缓存和公共组件复查等问题沉淀到 `AIGC.Framework/workflows/game-development-lifecycle/`。
- 已将原 WorldSeed 工程迁移为 `Experiments/EXP-001-worldseed/` 实验项目，通用框架迁移为 `AIGC.Framework/`。

## 当前事实

- 当前仓库包含 Godot 工程入口 `Experiments/EXP-001-worldseed/project/project.godot`。
- 当前仓库包含第一阶段 GameSpec 样例数据和 Godot .NET/C# 运行时原型。
- 当前运行时入口文件为 `project/scripts/Main.cs`、`project/scripts/GameRuntime.cs`、`project/scripts/GameSpecLoader.cs`。
- 当前 C# 项目文件为 `project/WorldSeed.csproj`，Godot assembly 名称为 `WorldSeed`。
...
继续基于 `TASK-20260516-006-gameplay-ui-alignment-rework` 做完整界面验收，重点检查斗法以外的奖励、构筑、奇遇、商店和结算界面是否仍存在与 v0.3 目标图不一致的布局问题；开发命令默认在 `Experiments/EXP-001-worldseed/project/` 下执行。
