...
- 加固 `game-development-lifecycle` 调试规则，补充 UI 同帧清空重建、延迟销毁和 Godot `RemoveChild` + `QueueFree` 的生命周期检查。
- 加固 `project-wiki-update` 规则，要求项目 wiki 更新时同步执行 CHG/VER/INDEX 编号、文件存在性和互相引用闭合检查。
- 更新 WorldSeed 追溯规则和 ID 规则，明确新建记录前必须查最大编号，禁止复用编号、索引悬空和同一语义重复登记。
- 扩展 THarness 回归测试，覆盖 UI 节点生命周期规则和项目 wiki 追溯闭合规则。
- 版本级别为 minor，因为本次改变现有工作流的必检行为，不改变项目事实写入边界。

## 2.6.0

- 加固 `game-iteration` 工作流，明确玩法策划案不能直接替代 UI 工程规格。
- 新增 `ui-ux-development-spec.md` 规则，用于把玩家界面拆分补成可直接指导 UI 开发的目标视口、缩放策略、布局合同、组件规范、hitbox、素材边界和验收标准。
- 新增 UI/UX 可开发规格模板，承接玩家界面总表、界面布局合同、交互和 hitbox 合同、核心组件规范、视觉拆解、素材边界和验收标准。
- 更新开发门控规则，要求进入 UI 或完整游戏实现前必须具备 UI/UX 可开发规格。
- 扩展 THarness 回归测试，确保 `game-iteration` 能路由到 UI/UX 可开发规格规则和模板。
- 版本级别为 minor，因为本次新增规则和模板，不改变项目事实写入边界。

## 2.5.0

- 补充游戏开发生命周期的项目侧执行同步经验，要求通用工作流升级或项目交付后回查目标项目适配层。
- 新增项目侧执行同步检查模板，覆盖当前工作流、状态页、构建运行入口、任务索引、运行记录和 CHG/VER 追溯。
- 扩展阶段门控和验证复盘规则，避免通用流程已更新但目标项目入口、任务状态或验证入口仍停留在旧版本。
- 增加 THarness 回归测试，检查项目侧同步模板和规则字段可被路由。
- 版本级别为 minor，因为本次新增通用模板和门控检查，不改变项目事实写入边界。

## 2.4.0

- 将 `game-development-lifecycle` 登记为通用能力，用于覆盖游戏从准备、实现、视觉校准、运行时排查、验证到复盘的完整闭环。
- 新增开发前硬门禁模板，集中核对工具链、目标语言、逻辑分辨率、缩放策略、界面合同、素材合同、数据蓝图、任务契约和验证命令。
- 新增素材管线合同模板，要求记录源资源、运行时资源、生成脚本、manifest 或等价索引、引擎导入缓存刷新和缺失策略。
- 扩展界面合同模板，补充关键控件 hitbox、最小点击尺寸、缩放后规则、真实输入验证、禁止遮挡区域和滚动策略。
- 扩展验证复盘模板，补充交互验证和关键控件证据，避免用截图替代真实点击、拖拽或跳转验收。
- 增加 THarness 回归测试，检查游戏开发生命周期工作流的硬门禁、素材管线、hitbox 和交互验证模板字段。
- 版本级别为 minor，因为本次新增通用能力登记、模板和门控检查，不改变项目事实写入边界。

## 2.3.0

- 新增 `game-iteration` 通用工作流，用于把游戏想法推进到可持续迭代流程。
- 新增输入边界、最小可玩循环与界面拆分、游戏数据蓝图、开发门控四条规则。
- 新增游戏迭代输入摘要、最小可玩循环和界面拆分、完整游戏数据蓝图、开发门控计划四个模板。
- 更新工作流索引和能力索引，使游戏项目可以从策划讨论衔接到数据蓝图、开发门控，再交接 `development` 工作流。
- 版本级别为 minor，因为本次新增通用工作流、规则和模板，不改变项目事实写入边界。
...
- 新增 `python tools\oneharness.py index --write`，按扫描结果写回 `wiki/index.yaml` 的页面清单。
- 新增 `python tools\oneharness.py self-check --path ... --delivery`，按变更路径输出应运行的自检命令。
- 扩展 `oneharness.yaml`，将自检路径规则和交付前命令移入配置。
- 将 wiki 元数据检查补齐到 `type` 和 `status` 字段，与 `wiki/SCHEMA.md` 保持一致。
...
- 新增 `oneharness.yaml`，集中配置入口文件、wiki 索引、规则元数据和质量门控检查范围。
- 为 `workflows/**/rules/*.md` 补充最小 YAML 元数据，使工作流规则可被机器检查。
- 扩展 `wiki/index.yaml`，加入显式页面清单，支持索引一致性校验。
...
- 修正根 README 的旧 wiki 路径，默认入口改为新版 `INDEX.md`。
...
- 明确通用 wiki 与项目 wiki 的调用边界：通用架构知识读 `wiki`，项目事实读目标项目适配层 wiki。
