...
- 项目 wiki 只保存到目标项目适配层，不保存到通用 `wiki`。
...
- `python tools/oneharness.py index --check` 可检查 `wiki/index.yaml` 页面清单一致性。
...
- `python tools/oneharness.py self-check --path wiki/architecture/entry-map.md --delivery` 可输出 `index --check` 和 `gate`。
